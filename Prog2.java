public class Prog2 {
    public static void main(String[] args) {
        /*
        Following is a function that contains a string of records, these records has product ID as the first value
        each record string which has commas between its values. The function will extract these ID's and print them
        each in one line
         */
        ...
        String[] recordSplits = records.split(":"); // Splits string into records, each record has an ID and other data

        System.out.println("product id's:"); //Print header
        for (String record : recordSplits) { //Loop over records
            String[] subArrColumns = record.split(","); // Split each record by comma
            String productId = subArrColumns[0]; // Get the product ID from each record
            System.out.println(productId); //Print product ID
        }
    }
}